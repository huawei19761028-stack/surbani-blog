...
사진만 첨부하고 생성하면, **Claude 비전이 사진을 분석**해 칼·가위의 종류와 상태(마모·날벌어짐·전후 차이 등)를
파악하고 그 내용을 반영한 글을 씁니다. (주제를 비워도 사진에서 자동 추론)

- 사진은 업로드 시 자동으로 리사이즈(최대 1024px)되어 분석에 사용됩니다.
- 사진 자체는 **저장되지 않습니다**(localStorage 에는 글 텍스트만 저장).
- 본문에는 `[사진: 설명]` 위치 마커가 삽입되니, 네이버에 붙여넣은 뒤 해당 위치에 실제 사진을 올리세요.
